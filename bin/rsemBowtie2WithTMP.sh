...
#@5,1,iso
rsem-generate-data-matrix $isoresults > iso.count.matrix && rsem-generate-data-matrix-tpm  out/*isoforms.results > iso.tpm.matrix.txt && rsem-run-ebseq --ngvector $index.ngvec iso.count.matrix ${counts#,} all.iso.results && rsem-control-fdr all.iso.results 0.05 final.iso.results.fdr.txt

#@6,1,gene
rsem-generate-data-matrix $generesults > gene.count.matrix && rsem-generate-data-matrix-tpm  out/*genes.results > gene.tpm.matrix.txt && rsem-run-ebseq gene.count.matrix ${counts#,} all.gene.results && rsem-control-fdr all.gene.results 0.05 final.gene.results.fdr.txt
