...
usage() { echo -e "\nUsage: \n$0 <old_bash_script.sh> <new_hpc_pipeline.sh> <bsub/sbatch options, such as: \"bsub -W 30:00 -q mcore -n 4\" or \"sbatch -p long -t 24:0:0 -n 4\" . Notice: it should be double quoted.>"; exit 1; } 
...
[ "$4" == "useTmp" ] && echo "binPath=`dirname \$0`" >> $run

#[ "$4" == "useTmp" ] && echo ". $binPath/rsyncToTmpAndRemoveFromTmp.sh" >> $run
...
for t in `cat $1`; do 
	...
	echo -e "\n${space}echo; echo step: $step, depends on: $de, job name: $name, flag: $name$loper $useTmp " >> $run
        ...
        [[ "$3" == sbatch* ]] && echo "${space}id=\$(sbatchRun \$xsub -flag \$deps \$cwd \$flag \"$cmd\") $ref"   >> $run
        [[ "$3" == bsub* ]] && echo "${space}id=\$(bsubRun \$xsub -flag \$deps \$cwd \$flag \"$cmd\") $ref"   >> $run
    ###########
        for t in ${ref//./ }; do
		echo "rsyncToTmp $t; $cmd; "  
	done
		exit 
